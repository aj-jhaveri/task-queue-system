...
### Q4: Why does `email.processor.ts` not call a live email vendor?
* **Answer:** The processor performs the full job lifecycle—validation, idempotency check, side-effect, durable success record—without a third-party vendor account, so the queue mechanics can be exercised without external credentials. It contains **no** artificial failure or delay switches: earlier revisions accepted `simulateFailure` and `delayMs` as API fields, which let any unauthenticated caller force three retry attempts plus a DLQ write from one HTTP request (and, via `delayMs`, hold worker slots open). Both fields were removed. Retry, backoff, and DLQ behavior is proven by `WEBHOOK_DELIVERY`, which issues a genuine HTTP `POST` to a path the service deliberately does not serve, and is additionally verified in the test suite by injecting real dependency errors.
...
* **Why not let the caller pass a URL?** That would be an SSRF hole on an unauthenticated endpoint: a caller could aim the worker at cloud metadata endpoints or internal services. Callers pick a destination from an enum instead, and the mapping to a real address happens server-side, so no caller-supplied value ever reaches the HTTP client. The defense is that no URL-shaped input exists, not that one is filtered.
