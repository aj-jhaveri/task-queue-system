...
* **What does it cost in practice?** One refresh costs a measured 27 Redis commands across the two queues. With the cache warm, a 106-second window of polling every 10 seconds cost exactly one refresh — the other nine polls reached Redis zero times. The worst case is bounded by the backstop rather than by traffic: 96 refreshes/day (~2,590 commands/day, ~78,000/month) even with a tab left open permanently, against ~14M/month for the uncached equivalent. Measuring that 27 rather than assuming ~10 is what prompted raising the backstop from 5 to 15 minutes; at 5 minutes a single forgotten tab would have cost ~233,000/month.
