...
* **Why rewrite the request instead of just the cache key?** Because canonicalizing only the key is a cache-poisoning bug, which is exactly what the first attempt at this fix shipped into the working tree. If `?page=999` still reaches Bull Board, it builds a page-999 payload that gets stored under the canonical `page=1` key, and the next visitor asking for page 1 receives page 999's empty result for a full TTL — one crafted URL corrupting the board for everyone. The canonical values are written back onto `req.query`, so the cached body and the key it lives under always describe the same view. The lesson generalizes: a cache key is a claim that two requests are equivalent, and the claim is only true if the *response* is equivalent too.
